...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(
        ...
        custom_config = AutoConfig.from_pretrained(mert_path, trust_remote_code=True)
        custom_config.conv_pos_batch_norm = True
        self.mert_model = AutoModel.from_pretrained(
            mert_path, config=custom_config, trust_remote_code=True
        ).to(self.device)
        self.mert_processor = Wav2Vec2FeatureExtractor.from_pretrained(
            mert_path, trust_remote_code=True
        )
        self.mu_mert_agg = nn.Conv1d(in_channels=25, out_channels=1, kernel_size=1).to(
            self.device
        )
        self.mu_mert_proj = nn.Linear(1024, 4096).to(self.device)
        ...
        self.mu_mert_norm_1 = bridge_norm_layer(4096).to(self.device)
        self.mu_mert_f1_1 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)
        self.mu_mert_f2_1 = nn.Linear(4096 * 4, 4096, bias=bridge_bias).to(self.device)
        self.mu_mert_f3_1 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)

        self.mu_mert_norm_2 = bridge_norm_layer(4096).to(self.device)
        self.mu_mert_f1_2 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)
        self.mu_mert_f2_2 = nn.Linear(4096 * 4, 4096, bias=bridge_bias).to(self.device)
        self.mu_mert_f3_2 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)

        self.mu_mert_norm_3 = bridge_norm_layer(4096).to(self.device)
        self.mu_mert_f1_3 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)
        self.mu_mert_f2_3 = nn.Linear(4096 * 4, 4096, bias=bridge_bias).to(self.device)
        self.mu_mert_f3_3 = nn.Linear(4096, 4096 * 4, bias=bridge_bias).to(self.device)
        ...
        self.llama = Transformer(model_args).to(self.device)  # Put LLaMA on second GPU
        ...
        for i, ckpt in enumerate(ckpts):
            ...
            for parameter_name, parameter in self.llama.named_parameters():
                ...
                if (
                    "gate" in parameter_name
                    or "lora" in parameter_name
                    or "bias" in parameter_name
                ):
                    continue
        ...
        self.query_layer = 20
        self.query_len = 1
        self.prefix_query = nn.Embedding(
            self.query_layer * self.query_len, model_args.dim
        ).to(self.device)  # Put prefix query on GPU 1 since it's used with LLaMA
        ...
        if knn:
            ...
            self.index = faiss.read_index(
                download(
                    "https://huggingface.co/csuhan/knn/resolve/main/knn.index", knn_dir
                )
            )
    ...
    def encode_audio(self, x):
        ...
        for sub_x in x:
            all_inputs = [
                self.mert_processor(
                    sub_x[
                        ix * self.mert_processor.sampling_rate : min(
                ...
                for ix in range(
                    0, len(sub_x) // (self.mert_processor.sampling_rate * 60) + 1, 60
                )
            ]
    ...
    def forward_audio(self, inputs, cache_size=10, cache_t=20, cache_weight=0.5):
        ...
        if self.knn:
            ...
            audio_feats = (
                1 - cache_weight
            ) * audio_feats_ori + cache_weight * audio_feats
            audio_feats = audio_feats / audio_feats.norm(dim=-1, keepdim=True)
        ...
        audio_feats = audio_feats + self.mu_mert_f2_1(
            F.silu(self.mu_mert_f1_1(audio_feats_norm))
            * self.mu_mert_f3_1(audio_feats_norm)
        )

        audio_feats_norm = self.mu_mert_norm_2(audio_feats)
        audio_feats = audio_feats + self.mu_mert_f2_2(
            F.silu(self.mu_mert_f1_2(audio_feats_norm))
            * self.mu_mert_f3_2(audio_feats_norm)
        )

        audio_feats_norm = self.mu_mert_norm_3(audio_feats)
        audio_feats = audio_feats + self.mu_mert_f2_3(
            F.silu(self.mu_mert_f1_3(audio_feats_norm))
            * self.mu_mert_f3_3(audio_feats_norm)
        )
        audio_feats = audio_feats.to(
            self.device
        )  # Move to GPU 1 before LLaMA processing
        return audio_feats

    @torch.inference_mode()
    def forward_inference(self, audio_feats, tokens, start_pos: int):
        tokens = tokens.to(self.device)  # Ensure tokens are on GPU 1
        ...
        for layer in self.llama.layers[-1 * self.query_layer :]:
            h = layer(
                ...
                freqs_cis,
                mask,
                audio_proj + prefix_query[prefix_index].repeat(_bsz, 1, 1),
            )
    ...
    def forward(self, tokens, labels, imgs):
        ...
        if labels.sum() == 0:
            ...
            c_loss = self.criterion(
                output.reshape(-1, self.llama.vocab_size), labels.flatten()
            )
