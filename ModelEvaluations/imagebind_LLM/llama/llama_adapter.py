...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(
        ...
        if knn:
            ...
            self.index = faiss.read_index(
                download(
                    "https://huggingface.co/csuhan/knn/resolve/main/knn.index", "ckpts"
                )
            )
        ...
        if phase == "finetune":
            ...
            for name, para in self.named_parameters():
                if name.startswith("llama."):
                    ...
                    "image_bind_"
                ):  # not 'image_bind.' so image_bind won't be trained.
                    trainable[name] = para
    ...
    def forward_visual(self, inputs, cache_size=10, cache_t=20, cache_weight=0.5):
        ...
        if self.knn:
            ...
            visual_feats = (
                1 - cache_weight
            ) * visual_feats_ori + cache_weight * visual_feats
            visual_feats = visual_feats / visual_feats.norm(dim=-1, keepdim=True)
        ...
        visual_feats = visual_feats + self.image_bind_f2_1(
            F.silu(self.image_bind_f1_1(visual_feats_norm))
            * self.image_bind_f3_1(visual_feats_norm)
        )

        visual_feats_norm = self.image_bind_norm_2(visual_feats)
        visual_feats = visual_feats + self.image_bind_f2_2(
            F.silu(self.image_bind_f1_2(visual_feats_norm))
            * self.image_bind_f3_2(visual_feats_norm)
        )

        visual_feats_norm = self.image_bind_norm_3(visual_feats)
        visual_feats = visual_feats + self.image_bind_f2_3(
            F.silu(self.image_bind_f1_3(visual_feats_norm))
            * self.image_bind_f3_3(visual_feats_norm)
        )
        return visual_feats
    ...
    def forward_inference(self, visual_feats, tokens, start_pos: int):
        ...
        for layer in self.llama.layers[-1 * self.query_layer :]:
            h = layer(
                ...
                freqs_cis,
                mask,
                visual_proj + prefix_query[prefix_index].repeat(_bsz, 1, 1),
            )
    ...
    def generate(
        ...
        with torch.amp.autocast():
            visual_query = self.forward_visual(
        ...
        for cur_pos in range(start_pos, total_len):
            with torch.amp.autocast():
                logits = self.forward_inference(
                    visual_query, tokens[:, prev_pos:cur_pos], prev_pos
                )
            if temperature > 0:
...
def load(
    ...
    adapter_ckpt = torch.load(model_path, map_location="gpu", weights_only=False)
